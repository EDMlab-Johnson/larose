...
def larose_over_time(crest_height_upgrade, time_step = 10, 
                     total_rise = 0.46, intensity_change = -.14, frequency_change = .125, 
    ...
    for time in years_from_start:
        
        sea_lvl = (slr_b * (time ** 2) + (slr_a * time)) * (3.28084) + slv_0
        intensity = (intensity_change / planning_horizon) * time
        frequency_mod = (frequency_change / planning_horizon) * time
        
        if time == 0:
            initial_ead, total_costs = lfv.larose_future_updated(sea_lvl, intensity, frequency_mod, crest_height_upgrade, needed_arguments, 1, planning_horizon, discount_rate)
            ead_list.append(initial_ead)
        else:
            ead, cost = lfv.larose_future_updated(sea_lvl, intensity, frequency_mod, crest_height_upgrade, needed_arguments, 0, planning_horizon, discount_rate)
...
pareto_frontier = optimize(model, "NSGAII", 50)
